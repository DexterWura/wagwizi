...
class Pesepay
{
    ...
    public function pollTransaction($pollUrl)
    {
        ...
        if (! is_string($decryptedData)) {
            return new ErrorResponse('Could not decrypt poll response.');
        }

        $jsonDecoded = json_decode($decryptedData, true);
        if (! is_array($jsonDecoded)) {
            return new ErrorResponse('Invalid poll response payload.');
        }
    ...
    public function initiateTransaction($transaction)
    {
        ...
        $jsonDecoded = json_decode($decryptedData, true);
        if (! is_array($jsonDecoded)) {
            return new ErrorResponse('Invalid initiate response payload.');
        }
        $referenceNumber = $jsonDecoded['referenceNumber'] ?? '';
        $pollUrl = $jsonDecoded['pollUrl'] ?? '';
        $redirectUrl = $jsonDecoded['redirectUrl'] ?? null;
    ...
    public function decodeCallbackPayload(string $cipherText): ?array
    {
        $decoded = $this->decrypt($cipherText);
